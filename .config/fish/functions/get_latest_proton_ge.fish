function get_latest_proton_ge
	...
	set tarball_url $(curl -s https://api.github.com/repos/GloriousEggroll/proton-ge-custom/releases/latest | grep browser_download_url | cut -d\" -f4 | grep .tar.gz)
	set tarball_name $(basename $tarball_url)
	echo "Downloading tarball: $tarball_name..."
	curl --progress-bar -L $tarball_url -o $tarball_name --no-progress-meter

	# Download the checksum for the latest release
	echo "Fetching checksum URL..."
	set checksum_url $(curl -s https://api.github.com/repos/GloriousEggroll/proton-ge-custom/releases/latest | grep browser_download_url | cut -d\" -f4 | grep .sha512sum)
